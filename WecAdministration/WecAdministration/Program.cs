...
namespace WecAdministration
{
    ...
    class Program
    {
        ...
        public static extern bool EcGetSubscriptionProperty(
            IntPtr Subscription,
            EC_SUBSCRIPTION_PROPERTY_ID PropetyId,
            Int32 Flags,
        ...
        private static  List<string> listSubscriptionRuntimeEventSources(string subscriptionName)
        {
            ...
            bool getProp = EcGetSubscriptionRunTimeStatus(
                subscriptionName,
                EC_SUBSCRIPTION_RUNTIME_STATUS_INFO_ID.EcSubscriptionRunTimeStatusEventSources,
                null,
                0, // pass in zero - docs say pass in NULL (is reserved)
                bufferSize,
                outputBuffer,
            ...
            if (lastError == ERROR_INSUFFICIENT_BUFFER)
            {
                ...
                if (getProp)
                {
                    ...
                    // event sources are returned as an array of pointers.
                    // enumerate over each pointer and dereference them to get the event source name (machine name)
                    int length = (int)results.Count;
                    for (int i = 0; i < length; i++)
                    {
                        // [pointer arithmetic] to move along the array, pointer by pointer.
                        IntPtr newPtr = IntPtr.Add(results.StringArr, (IntPtr.Size * i)); 
